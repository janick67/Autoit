...
_FFStart("https://pl116.plemiona.pl/game.php?village=37287&screen=place",0,2)
start()


Func Start()
	If _FFIsConnected() Then
		$lista2 = FileRead("nwioski.txt")
$tab = StringSplit($lista2, ";")
_ArrayDisplay($tab)
for $i = 1 to UBound($tab) - 1
	$info = $i&"/"&UBound($tab) - 1
	_FFSetValue(1,"unit_input_spy","id")
	_FFSetValue($tab[$i],"target-input-field","class")
	Sleep(Random(110,300))
	_FFClick("target_attack","id")
	Sleep(Random(110,300))
	_FFClick("troop_confirm_go","id")
	Sleep(Random(110,900))
	Next
EndIf
EndFunc
