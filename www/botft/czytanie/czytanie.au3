...
$kod = "GET /trening_normal.php HTTP/1.1" & @CRLF & "Host:" & $host & @CRLF & "Cookie: " & $sesja & @CRLF & "Connection: keep-alive" & @CRLF & @CRLF
...
;~ MsgBox(0,"",$kod)
	$re = hsend($host,$kod)
	$p = StringInStr($re,'zu¿y³e¶ ')+8
	$k = StringInStr($re,'/',0,1,$p)
	$ntdzis = int(StringMid($re,$p,$k-$p))
	MsgBox(0,"",$ntdzis)
