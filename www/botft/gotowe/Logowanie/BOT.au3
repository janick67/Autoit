...
dim $Tnum[10]= ["Atak","Obrona","Podania","Bronienie","Kondycja","Skutecznoœæ","Szybkoœæ","Celnoœæ","Doœrodkowania","Strza³y"];Tablica przechowuj¹ca nazwy umiejêtnoœci
...
Gi();funkcja tworz¹ca GUI z informacjami
GUICtrlCreateTabItem("Trening");Tworzenie zakladki nr 2
;~ Gtn()
;~ $GLum1 = GUICtrlCreateLabel("UM 1:",5,27,35,17);Label z tytu³em do pierwszej umiejêtnoœci
;~ $Gum1 = GUICtrlCreateCombo("Obrona", 40, 25, 100, 20);Wybór umiejêtnoœci
...
;~ $GLum2 = GUICtrlCreateLabel("UM 2:",145,27,35,17);Label z tytu³em do drugiej umiejêtnoœci
;~ $Gum2 = GUICtrlCreateCombo("Skutecznoœæ", 180, 25, 100, 20);Wybór 2 umiejêtnoœci
...
;~ $GLile = GUICtrlCreateLabel("Ile PA z³u¿yæ:",290,27,65,17);Label z tytu³em do iloœci treningu
;~ $Gile = GUICtrlCreateInput("150", 360, 25, 50, 20, BitOR($ES_AUTOHSCROLL,$ES_NUMBER));Input z liczb¹ PA które maj¹ byæ wytrenowane
...
func spr()
	if $Cstaty = 1 and int(TimerDiff($Drestaty)/1000) >= 120 then 
		loadstaty();Odœwie¿a je¿eli od poprzedniego odœwie¿enia minê³y 2 min.
		setinfo();Ustawia w GUI pobrane dane
	ElseIf $Ctop = 1 and int(TimerDiff($Dreload)/1000) >= 20 then
...
global $Dreload = TimerInit();Zapisz do zmiennej czas od ostatniego odœwie¿enia
	EndFunc
...
global $Drestaty = TimerInit();Czas od ostatniego odœwie¿enia
	EndFunc