...
global $Ztrick_combo = "", $Gtrick, $Gtrick_b
Func Gtrick()
...
next
if $Ztrick_combo = "" Then;Je¿eli jeszcze nie by³o nic przypisane to:
for $i=0 to 24;Tricki
	if $Ttrick[$i][2] < 11 Then;Je¿eli jeszcze nie ukoñczono tricku to dopisuje go do listy
	$Ztrick_combo &= "|"&$Ttrick[$i][0]
	EndIf
Next
$Ztrick_combo = StringTrimLeft($Ztrick_combo,1);Odcina pierwszy znak
GUICtrlSetData($Gtrick, $Ztrick_combo);Ustawia now¹ liste
EndIf
...
func trick_spr();Sprawdza czy s¹ wiadomoœci z treningu
	...
	if $autor <> -1 Then;Je¿eli coœ znalaz³o to:
		for $i = 0 to UBound($autor)-1;Wszystkie znalezione elementy
			if $Tpost[$autor[$i]][3] = "Raport z treningu" then;Wiadomoœæ o temacie Raport z treningu
...
Func trick_read($postid);Czyta raport z treningu o podanym id
$kod = "GET /poczta_ajax.php?Step=Read&id="&$postid&" HTTP/1.1" & @CRLF & "Host:" & $host & @CRLF & "Cookie: " & $Zsesja & @CRLF & "Connection: keep-alive" & @CRLF & @CRLF
...
$w2 = StringReplace($w2,".","")
$Twsp[0][1] = $w2;Poziom
for $i = 1 to $Twsp[0][1]*2;Poziom = iloœæ wsp
