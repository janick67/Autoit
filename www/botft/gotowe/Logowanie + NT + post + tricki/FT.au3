...
func spr();Funkcja w której s¹ funckcje wykonywane co pewien czas
	...
	ElseIf $Ctop = 1 and int(TimerDiff($reload_time)/1000) >= 20 then
	...
	ElseIf $Ctrick = 1 and _ArraySearch($Ttrick,IniRead($Ztrick_file,"ostatni","trick",""),0,0,0,0,1,0) > -1 and IniRead($Ztrick_file,"ostatni","czas","") <> "" and TimerDiff(IniRead($Ztrick_file,"ostatni","czas",""))/60000 >= IniRead($Ztrick_file,"ostatni","poziom",10)*5 Then
		trick_learn(_ArraySearch($Ttrick,IniRead($Ztrick_file,"ostatni","trick",""),0,0,0,0,1,0));Nauka ostatniego tricku
	EndIf
EndFunc;spr
...
func start2();Funkcja po stworzeniu GUI
	...
	If $Ctrick = 1 Then
		...
		if not FileExists($Ztrick_file) Then
			trick_file($Ztrick_file)
		Else
			trick_lvl_get()
		EndIf
