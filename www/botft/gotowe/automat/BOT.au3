...
$Zcombo1 = StringLeft($Ztrick_combol,StringInStr($Ztrick_combol,"|")-1)
$Zcombo2 = StringTrimLeft($Ztrick_combol,StringInStr($Ztrick_combol,"|"))
$Gtrick = GUICtrlCreateCombo($Zcombo1, 40, 25, 100, 20);Wybó
GUICtrlSetData(-1,$Zcombo2)
...
While 1 
	...
	Case $Gtrick_b
		FileDelete("t.ini")
		FileWrite("t.ini","[Ustawienia]"&@CRLF&"Nick="&$Tinfo[0][1]&@CRLF&"Email="&$Zemail&@CRLF&"Pass="&$Zpass&@CRLF&"Serw="&$Zserwer&@crlf&"Trick="&GUICtrlRead($Gtrick))
		RunWait("trick.exe")
		$re = FileRead("t.ini")
		FileDelete("t.ini")
		slog("Rozpoczêto trening tricku "&GUICtrlRead($Gtrick))
	Case $Gtask_tk
		MsgBox(0,"",task_tk())
	Case $Gtask_mp
