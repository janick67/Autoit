...
Next
	$ile = $Tpost[0][0] - $ile
	if $ile > 0 Then slog("Pobrano "&$ile&" wiadomoœci")
	post_show(1)
EndFunc;post_get
...
func post_delete($id);Usuwa wiadomoœæ
	$post = $id&"="&$id;Dane do przes³ania
	$kod = "POST /poczta_ajax.php?Step=Recive&DelSelect=True HTTP/1.1" & @CRLF & "Host: s5.footballteam.pl" & @CRLF & "Content-Type: application/x-www-form-urlencoded; charset=UTF-8" & @CRLF & "Content-Length: "& StringLen($post) & @CRLF & "Cookie: " & $Zsesja & @CRLF & "Connection: keep-alive" & @CRLF & @CRLF & $post & @CRLF
	hsend($host,$kod)
EndFunc