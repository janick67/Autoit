...
download()
FileWrite("cpog.html",clerindex(FileRead($index)))
Exit
...
ShellExecute("www.rdzawka.marianie.pl/indext.html")
Sleep(2000)
Send("{F5}")
Sleep(4000)
if MsgBox(4,"Index","Strona dobrze wygl¹da?")=6 Then
	_FTP_FileDelete($FTP_Conn,"index.html")
	If Not @error Then ConsoleWrite("-> File delete Success" & @LF)
	_FTP_FileRename($FTP_Conn,"indext.html","index.html")
	If Not @error Then ConsoleWrite("-> File rename Success" & @LF)
	_FTP_FileDelete($FTP_Conn,"indext.html")
	If Not @error Then ConsoleWrite("-> File delete Success" & @LF)
	ShellExecute("www.rdzawka.marianie.pl/index.html")	
	Sleep(1000)
	Send("{F5}")
Else
...
func upload()
   $FTP_Put = _FTP_FilePut($FTP_Conn,"index~~1.html","indext.html")
